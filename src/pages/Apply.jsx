...
export default function RequestForm(){
  ...
  useEffect(() => {
    if(location.state?.preselect){
      setForm(prev => ({...prev, creditId: location.state.preselect}))
    }
  }, [location])
  ...
  }, [form.monto, form.plazo, form.creditId])
       ...
       <form className= "form-grid" onSubmit={onSubmit} noValidate>
        ...
        <div className="form-group">
          ...
          <select value={form.plazo} onChange={onChange('plazo')}>
            {[12,24,36,48,60,72,84].map(n => <option key={n} value={n}>{n}</option>)}
          </select>
        </div>
        ...
        <div className="form-group full">
          <p> Cuota estimada:
            <strong> {formatCOP(Math.round(monthly))} </strong>  / mes
        ...
        <div className="form-group full buttons"> 
          <button className="btn-primary" type="button" onClick={() => setForm(initial)}> Limpiar </button>
        </div>
      </form>
    </main>
  )
